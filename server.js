...
function getAccessToken(req) {
  return req.session.accessToken || req.session.user?.access_token || null;
}
...
app.get('/api/', async (req, res) => {
  ...
  try {
    ...
    // セッションに保存（アクセストークンも保存）
    req.session.user = user;
    req.session.accessToken = tokenData.access_token;  // ← 追加！
    req.session.refreshToken = tokenData.refresh_token;  // ← 追加！
    console.log('✅ Session saved (with access token)');
  ...
  try {
    ...
    if (!req.session || !req.session.accessToken) {
      return res.status(401).json({
        error: 'Not logged in. Please login with Talknote OAuth first.',
        debug: {
          sessionExists: !!req.session,
          hasUser: !!req.session?.user,
          hasAccessToken: !!req.session?.accessToken,
          hint: 'Use Talknote OAuth login (not simple login) at /app'
        }
      });
    }
...
app.get('/api/requests/sent', requireAuth, async (req, res) => {
  try {
    const requests = await database.getSentRequests(req.session.user.id);
    res.json(requests);
  } catch (error) {
...
app.get('/api/requests/received', requireAuth, async (req, res) => {
  try {
    const requests = await database.getReceivedRequests(req.session.user.id);
    res.json(requests);
  } catch (error) {
    console.error('Received requests fetch error:', error);
...
app.post('/api/requests', requireAuth, async (req, res) => {
  try {
    ...
    // アクセストークンの取得（user.access_token または session.accessToken）
    const accessToken = req.session.user.access_token || req.session.accessToken;
    ...
    if (accessToken) {
      try {
        ...
        const talknoteAPI = new TalknoteAPI(accessToken);
      ...
      console.log('⚠️  Skipping Talknote notification: No access token available (simple login used)');
      notificationError = 'No access token (simple login)';
    }
...
app.patch('/api/requests/:id/status', requireAuth, async (req, res) => {
  try {
    ...
    await database.updateRequestStatus(req.params.id, status);
